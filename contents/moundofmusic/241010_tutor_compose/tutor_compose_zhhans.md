...
- 该文件用作曲目在选择界面和碟片上的图片，要求如下：
    - 文件名必须与索引名一致
    - 必须为png文件
    - 如上传创意工坊，该图片将自动作为封面使用。受Steam本身的限制，建议图片大小不超过1MB
...
## 其他
### 可转化midi单音轨文件为配置的工具
- 请直接在此下载工具`mom_converter`：https://github.com/gamefang/MOMTools/releases/download/v1.0/mom_converter.exe
- 转化midi文件
    - 准备完整曲目的midi文件
    - 提取其中的主旋律音轨，可使用[MuseScore](https://musescore.org/en/download)删除其他音轨。如果主旋律分布在多个音轨，则此方法可能不太合适。
    - 导出主旋律midi文件，执行`mom_converter`，将midi文件拖入并执行
    - 如果提示成功，则会生成`note_raw.csv`文件，此文件与[note.csv](#notecsv)格式一致
